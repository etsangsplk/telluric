...
instr_encoded encode_instr(instr in) {
  ...
  if (in.has_return) {
    ...
    return_operand = calloc(in.return_operand.value_len + 1, sizeof(*return_operand));
  ...
  int i = 0;
  o.bytes = calloc(total_len, sizeof(*o.bytes));
  if (in.has_return) {
...
object compile_code(code_section c, data_domains d) {
  ...
  o.instrs = calloc(c.instr_count * 2, sizeof(*o.instrs)); // double space for possible expansions
  o.instrs_length = 0;
